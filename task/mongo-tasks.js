...
async function before(db) {
    ...
    await db.collection('orders').createIndex({"CustomerID": 1, "OrderID" : 1});
    await db.collection('orders').createIndex({"CustomerID": 1});
    await db.collection('orders').createIndex({"EmployeeID": 1});
    await db.collection('order-details').createIndex({"OrderID" : 1,"Quantity": 1, "UnitPrice" : 1});
    await db
      .collection('customers')
      .createIndex({ CustomerID: 1, CompanyName: 1 });
}
        ...
        .aggregate([
            {
                $group: {
                    ...
                    TotalDiscount: {
                        $sum: {$multiply: ['$Quantity', '$Discount']}
                    }
                }
            },
            {
                $project: {
                    ...
                    'Total Order Discount, %': {
                        $round: [
                            {
                                $multiply: [{ $divide: ['$TotalDiscount', '$TotalPrice'] },100]
                            },
                        3]
                    }
                }
...
async function task_1_4(db) {
    let countOrders = await db.collection('orders').countDocuments();
                ...
                $project: {
                    ...
                    '% of all orders': {
                        $round: [
                            {
                            $divide: [
                                { $multiply: ['$TotalNumberOfOrders', 100] },
                                countOrders
                            ]
                            },
                            3
        ...
        .aggregate([
            ...
            },
            {
                $project: {
                    _id: 0,
                    CategoryName: 1,
                    TotalNumberOfProducts: { $size: '$Categories' }
                }
            },
            { $sort: { CategoryName: 1 } }
        ])
...
async function task_1_13(db) {
    let countProducts = await db.collection('products').countDocuments();
    const countDiscontinuedProducts = await db
        .collection('products')
        .countDocuments({ Discontinued: 1 });

    return {
        TotalOfCurrentProducts: countProducts,
        TotalOfDiscontinuedProducts: countDiscontinuedProducts
    };
}
...
async function task_1_22(db) {
    const result = await db
        ...
        .aggregate([
            ...
            { $unwind: '$MaxPriceOrder' },
            {
                $lookup: {
                    from: 'products',
                    let: { productID: '$MaxPriceOrder.ProductID' },
                    pipeline: [
                        {
            ...
            { $unwind: '$Product' },
            {
                $project: {
                    _id: 0,
                    CustomerID: '$MaxPriceOrder.CustomerID',
                    CompanyName: '$MaxPriceOrder.Customer.CompanyName',
                    ProductName: '$Product.ProductName',
                    PricePerItem: '$MaxPriceOrder.Price'
                }
            },
            {
                $sort: {
